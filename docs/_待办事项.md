...
## 配置管理
 ...
 1. 
现在配置管理界面引进了统一的Json序列化方式。 @MoLibrary.Configuration\Providers\JsonFileProviderConventions.cs \                                                                                           │
│   配置管理界面要以Json编辑为主，而不是编辑原始的object对象 @DtoOptionItem 

配置管理界面的 @ConfigurationEditor.razor 需要修复：
1. 修改组件有些类型没有读取成功现有的值，是默认值，比如Numeric类型，请你检查并优化相关的类型。
