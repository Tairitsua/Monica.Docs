...
## 配置管理
  ...
  相关界面代码：@d:\Euynac\Programming\.NET\Euynac\Monica\Monica.Configuration.Dashboard\Pages\
  @d:\Euynac\Programming\.NET\Euynac\Monica\Monica.Configuration.Dashboard\UIConfiguration

  设计文档详见：@d:\Euynac\Programming\.NET\Euynac\Monica.Docs\docs\Configuration\MoConfigurationDashboard.md
  注意查看新的MudBlazor迁移文档
...
```
Monica.Generators.AutoController 项目之前实现了Handler的自动接口生成， 我现在需要在其中增加一个生成 Client 侧RPC接口实现的代码的功能，具体是判断项目是否是使用了 AutoControllerGeneratorClientConfigAttribute 标签 （这些基础架构相关项目在 ../Monica 下），要同时支持gRPC实现，目前先实现HTTP，预留gRPC扩展无需实现。
...
```markdown
 Monica 主题优化指南
  ...
  Monica 使用双层分离架构：C# 主题层负责颜色定义，CSS 层负责样式效果。目标是优化现有主题，遵循
  ...
  - 找到主题文件：Monica.UI\Themes\Theme[主题名].cs 和 Monica.UI\wwwroot\css\themes\mo-theme-[主题名].css
...
var jwtConfig = provider.GetService<IOptions<JwtTokenOptions>>()!.Value;
...
```
 "ExceptionObject": "[FORCE]System.AggregateException: Some services are not able to be constructed (Error while validating the service descriptor 'ServiceType: Monica.Framework.Services.IFrameworkMonitorService Lifetime: Scoped ImplementationType: Monica.Framework.Services.FrameworkMonitorService': Unable to resolve service for type 'Monica.EventBus.Abstractions.IMoDistributedEventBus' while attempting to activate 'Monica.Framework.Services.FrameworkMonitorService'.) (Error while validating the service descriptor 'ServiceType: Monica.Configuration.Dashboard.UIConfiguration.Services.ConfigurationClientService Lifetime: Scoped ImplementationType: Monica.Configuration.Dashboard.UIConfiguration.Services.ConfigurationClientService': Unable to resolve service for type 'Monica.Configuration.Dashboard.Interfaces.IMoConfigurationModifier' while attempting to activate 'Monica.Configuration.Dashboard.UIConfiguration.Services.ConfigurationClientService'.) (Error while validating the service descriptor 'ServiceType: Monica.Configuration.Dashboard.UIConfiguration.Services.ConfigurationDashboardService Lifetime: Scoped ImplementationType: Monica.Configuration.Dashboard.UIConfiguration.Services.ConfigurationDashboardService': Unable to resolve service for type 'Monica.Configuration.Dashboard.Interfaces.IMoConfigurationCentre' while attempting to activate 'Monica.Configuration.Dashboard.UIConfiguration.Services.ConfigurationDashboardService'.) (Error while validating the service descriptor 'ServiceType: Monica.UI.Services.MoThemeService Lifetime: Singleton ImplementationType: Monica.UI.Services.MoThemeService': Cannot consume scoped service 'Microsoft.Extensions.Options.IOptionsSnapshot`1[Monica.UI.Modules.ModuleUICoreOption]' from singleton 'Monica.UI.Services.MoThemeService'.) (Error while validating the service descriptor 'ServiceType: Monica.SignalR.Controllers.ModuleSignalRController Lifetime: Transient ImplementationType: Monica.SignalR.Controllers.ModuleSignalRController': Unable to resolve service for type 'Monica.SignalR.Controllers.MoSignalRManageService' while attempting to activate 'Monica.SignalR.Controllers.ModuleSignalRController'.)
 ---> System.InvalidOperationException: Error while validating the service descriptor 'ServiceType: Monica.Framework.Services.IFrameworkMonitorService Lifetime: Scoped ImplementationType: Monica.Framework.Services.FrameworkMonitorService': Unable to resolve service for type 'Monica.EventBus.Abstractions.IMoDistributedEventBus' while attempting to activate 'Monica.Framework.Services.FrameworkMonitorService'.

```


### ConfiguratioUI不支持单体架构