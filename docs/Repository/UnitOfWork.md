...
工作单元（Unit of Work，简称UOW）是一种设计模式，用于维护一组在事务边界内执行的操作列表。Monica实现了工作单元模式，提供了一种优雅的方式来确保数据库操作的原子性，同时简化了事务管理。
...
Monica的工作单元实现基于异步本地存储（AsyncLocal）来维护当前工作单元的上下文，使其在异步环境下正常工作。工作单元管理器会自动处理嵌套工作单元，确保事务正确传播。
