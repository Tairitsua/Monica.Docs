...
| `AppConfiguration` | `IConfigurationManager?` | `builder.Configuration` | 否 | 需要替换为自定义配置管理器时 | 默认由模块在注册阶段从宿主 `IHostApplicationBuilder.Configuration` 取得。 |
...
| `SetOtherSourceAction` | `Action<IConfigurationManager>?` | `null` | 否 | 你还要追加更高优先级的配置源时 | 后读入的配置源优先级更高。 |

## Required setup

`Configuration` 模块没有必需 Guide 配置。常规宿主只需要调用 `Mo.AddConfiguration()`，模块会自动使用 `builder.Configuration`。
