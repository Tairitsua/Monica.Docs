...
`monica.manifest.json` 中的每个模块声明都有独立且全局唯一的生态键：

- 模块键必须等于 `PackageId`，或以 `PackageId.` 开头。
- 如果包存在一个根模块，可以直接使用包 ID 作为其模块键。
- 其他模块在包 ID 后追加简洁的功能名。
- 第三方 UI 模块键必须以精确的 `.UI` 片段结尾。
- 在 Manifest、README 与测试中保持同样的大小写。仓库工具以不区分大小写的方式比较 Manifest 键，并拒绝冲突。

Manifest 键描述分发归属，并让仓库工具把 `dependsOn` 条目解析为具体模块类型。Monica 运行时始终使用具体策略 `Type` 作为模块图身份；`ModuleKey` 是派生的诊断元数据，不是特性，也不是第二套身份系统。

对于 Manifest 键 `Tairitsua.Monica.GachaPool` 与 `Tairitsua.Monica.GachaPool.UI`，对应策略使用普通 CLR 类型：
...
`modules[].dependsOn` 通过完整 Manifest 键列出分发依赖。Scaffold 会把每个条目解析为 `Describe(ModuleDescriptor)` 中的 `module.Require<TModule, TOptions>()`。每个跨包模块依赖都要求归属包声明对应包依赖。Provider 模块要设置 `kind: provider`、实现 `IModuleProvider`，并通过 `providerFor` 命名所提供的能力；同一目标键必须出现在 `dependsOn` 中。包图与解析后的模块图都必须无环。
...
第三方 Module、Option 与注册扩展统一放在包自有的 `<PackageId>.Modules` 命名空间。宿主入口返回 `ModuleRegistration<TModule, TOptions>`，流式 Feature 方法继续扩展同一注册类型。消费者通过 `using Acme.Monica.Analytics.Modules` 引入对应发布者的注册入口。`Monica.Modules` 仅供 Monica 官方模块使用；否则两个独立发布者采用相同模块名时会生成完全相同的 CLR 类型名。
...
- 从 UI Manifest 键中只移除末尾 `.UI`，得到稳定分类 ID。因此 `Tairitsua.Monica.GachaPool.UI` 的分类 ID 是 `Tairitsua.Monica.GachaPool`，公开路由仍为 `/gacha-pool`。
...
一个 NuGet 包可以包含多个 UI 模块。每个模块都用自身 UI Manifest 键移除 `.UI` 后的值作为分类 ID，因此 `Acme.Monica.Toolkit.Audit.UI` 与 `Acme.Monica.Toolkit.Admin.UI` 可以保留不同分类，而不必拆成多个包。所有贡献都在启动阶段完成；Shell 会在路由读取前冻结并验证注册表。
