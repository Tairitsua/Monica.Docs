...
`AddPermissionBit<TEnum>(claimType)` adds another permission family. `ConfigAsAlwaysAllow()` deliberately bypasses checks and should be limited to isolated development or tests. `AddAuthorizationInterceptor()` adds the DynamicProxy-based method-invocation check; endpoint policies still use normal ASP.NET Core authorization.
